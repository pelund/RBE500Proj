...
def handle_joint_vel_rev(req):
    ...
    J = numpy.array([[(-1*a1*numpy.sin(theta1)-a2*numpy.sin(theta1+theta2)), -1*a2*numpy.sin(theta1 + theta2), 1],[a1*numpy.cos(theta1)+a2*numpy.cos(theta1+theta2), a2 * numpy.cos(theta1 + theta2), 1] ,[1,1,-1]])
    JT = numpy.transpose(J)
    p = numpy.array([[vx],[vy],[vz]])
    q = numpy.multiply(JT,p)
    print('Jacobian transpose is: ', JT)
    vtheta1 = q[0,0]
    vtheta2 = q[1,0]
    vtheta3 = q[2,0]
