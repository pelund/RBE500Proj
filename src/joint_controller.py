...
class PD_Controller:
    ...
    def PD(self, current, desired):
        ...
        desired_file = open("/home/jibran/catkin_ws/src/rbe_proj/src/desired.txt", "a")
        desired_file.write(str(desired) + '\n')
        desired_file.close()

        current_file = open("/home/jibran/catkin_ws/src/rbe_proj/src/current.txt", "a")
        current_file.write(str(current) + '\n')
        current_file.close()

        delta_error = self.old_error - self.error
        calculated_effort = (Kp * self.error) + (Kd * delta_error)
        print(f'Desired: {desired} Current: {current} Error: {self.error} Delta_error {delta_error} Time {rospy.Time.now()} ')
...
if __name__ == '__main__':
    desired_file = open("/home/jibran/catkin_ws/src/rbe_proj/src/desired.txt", "r+")
    current_file = open("/home/jibran/catkin_ws/src/rbe_proj/src/current.txt", "r+")
